...
main (void) {
  ...
  //Lexicographic weighting is applied

      IloInt BigM =2;
      IloFloatArray W(env, m);
    ...
    // Y_i >= Y_i+1
      for (i = 0; i < m-1; i++){
      	  model.add(W[i+1]*Y[i] >= W[i+2]*Y[i+1]);
      }

    // Y_i >= (1/m-i+1)*(Sum(p_j)-Sum(Y_j_))
      for (i = 0; i < m; i++){
          IloExpr sumpj(env);
          IloExpr sumCibar(env);
          for (j = 0; j < n; j++){
        	  sumpj += P[j];
          }
          for (k = 1; k <= i-1; k++){
      	  ...
      	  model.add(W[i+1]*Y[i]*(m-i+1)+ sumCibar >= sumpj);
      	  sumpj.end();
      	  sumCibar.end();
      }

     // Y_i >= (1/m-i+1)*(Sum(p_j))-Sum(Y_j)))
      for (i = 0; i < m; i++){
          IloExpr sumpj(env);
          IloExpr sumCibar(env);
          ...
          for (k = i+1; k <= m; k++){
      	  ...
      	  model.add(sumpj >= i*W[i]*Y[i]+sumCibar);

      	  sumpj.end();
      	  sumCibar.end();
      }//REFORMULATING OF page 7 of the paper  
